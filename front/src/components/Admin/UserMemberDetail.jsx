                ...
                <table className="min-w-full table-fixed border-separate border-spacing-0">
                    <thead>
                        <tr className="bg-gradient-to-r from-[#845EC2] to-[#D65DB1]">
                            ...
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">가입일</th>
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">전화번호</th>
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">닉네임</th>
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">상태</th>
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">성별</th>
                            <th className="p-5 text-lg text-white font-bold border-b-2 border-[#F6F1FA]">등급</th>
                        </tr>
                    </thead>
                    <tbody>
                        {users.map(user => (
                            <tr key={user.id} className="even:bg-[#F6F1FA] odd:bg-white text-[#5B5B5B] hover:bg-[#E0CFFD] transition">
                                <td className="p-4 text-base text-[#845EC2] font-mono truncate max-w-[220px]">{user.id}</td>
                                <td className="p-4 text-base">{user.name}</td>
                                <td className="p-4 text-base">{user.email}</td>
                                <td className="p-4 text-base">{formatDate(user.joinDate)}</td>
                                <td className="p-4 text-base">{user.phone}</td>
                                <td className="p-4 text-base">{user.nickname}</td>
                                <td className="p-4 text-base font-semibold">{user.status}</td>
                                <td className="p-4 text-base">{user.gender}</td>
                                <td className="p-4 text-base">{user.tier}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
